...
// Windows absolute paths for D:\ root directory fix
...
const WIN_FINDSTR = `${WIN_SYS32}\\findstr.exe`;
...
export class WindowsStrategy implements platform_strategy {
	...
	get_process_list_command(process_name: string): string {
		if (this.use_powershell) {
			// Use absolute path for PowerShell to fix D:\ root directory issue
			return `"${WIN_POWERSHELL}" -NoProfile -Command "Get-CimInstance Win32_Process -Filter \\"name='${process_name}'\\" | Select-Object ProcessId,CommandLine | ConvertTo-Json"`;
		}
		// Use absolute path for WMIC
		return `"${WIN_WMIC}" process where "name='${process_name}'" get ProcessId,CommandLine /format:list`;
	}
	...
	get_port_list_command(pid: number): string {
		if (this.use_powershell) {
			// Use absolute path for PowerShell to fix D:\ root directory issue
			return `"${WIN_POWERSHELL}" -NoProfile -Command "Get-NetTCPConnection -OwningProcess ${pid} -State Listen | Select-Object -ExpandProperty LocalPort | ConvertTo-Json"`;
		}
		// Use absolute paths for netstat and findstr
		return `"${WIN_NETSTAT}" -ano | "${WIN_FINDSTR}" "${pid}"`;
	}
