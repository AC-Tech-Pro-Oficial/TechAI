...
class Router {
    constructor(profileManager, backendPool) {
    ...
    async handleRequest(request, context) {
        logger_1.logger.debug(LOG_CAT, `Request: ${request.method} from workspace ${context.workspaceId}`);
        try {
            ...
            logger_1.logger.error(LOG_CAT, `Error handling request:`, error);
            return this.createErrorResponse(request.id, -32603, error instanceof Error ? error.message : 'Internal error');
        }
    }
    /**
    ...
    async getOrCreateSession(context) {
        ...
        if (!session || session.workspacePath !== context.workspacePath) {
            ...
            session = {
                ...
                contextInjector: new context_1.ContextInjector(context.workspacePath),
                lastActivity: Date.now()
            };
            this.sessions.set(context.workspaceId, session);
            // Connect to required backend servers
            logger_1.logger.info(LOG_CAT, `Creating session for ${context.workspaceId} with profile "${profile.name}"`);
     ...
     */
    async ensureServersConnected(serverIds) {
        const toConnect = [];
        for (const id of serverIds) {
            if (this.backendPool.hasServer(id)) {
                ...
                logger_1.logger.warn(LOG_CAT, `Server ${id} not found in MCP config`);
            }
        }
        if (toConnect.length > 0) {
            logger_1.logger.info(LOG_CAT, `Connecting to ${toConnect.length} servers: ${toConnect.join(', ')}`);
     ...
     */
    handleInitialize(request, _session) {
        return {
            jsonrpc: '2.0',
            id: request.id,
            result: {
     ...
     */
    handleToolsList(request, session) {
        const tools = this.backendPool.getTools(session.enabledServers);
        logger_1.logger.debug(LOG_CAT, `Returning ${tools.length} tools for workspace ${session.workspacePath}`);
        return {
            jsonrpc: '2.0',
            id: request.id,
            result: { tools }
     ...
     */
    async handleToolsCall(request, _session) {
     ...
     */
    handleResourcesList(request, session) {
        ...
        logger_1.logger.debug(LOG_CAT, `Returning ${allResources.length} resources (${contextResources.length} context + ${backendResources.length} backend)`);
        return {
            jsonrpc: '2.0',
            id: request.id,
    ...
    async handleResourcesRead(request, session) {
        ...
        if (params.uri.startsWith('workspace://')) {
            try {
                ...
                return this.createErrorResponse(request.id, -32602, error instanceof Error ? error.message : 'Resource not found');
            }
        }
        else {
            // Read from backend
     ...
     */
    handlePromptsList(request, session) {
        const prompts = this.backendPool.getPrompts(session.enabledServers);
        logger_1.logger.debug(LOG_CAT, `Returning ${prompts.length} prompts`);
        return {
            jsonrpc: '2.0',
            id: request.id,
            result: { prompts }
     ...
     */
    async handlePromptsGet(request, _session) {
    ...
    cleanupStaleSessions() {
        ...
        for (const id of stale) {
            logger_1.logger.debug(LOG_CAT, `Removing stale session: ${id}`);
     ...
     */
    async refreshSession(workspaceId, workspacePath) {
