...
export class ToolFilter {
     ...
     */
    public filterByIntent(tools: ToolDefinition[], userMessage: string): ToolDefinition[] {
        ...
        if (filtered.length < 5) {
            logger.debug(LOG_CAT, `Filtering too aggressive, returning all ${tools.length} tools`);
            return tools;
        }
        
        logger.info(LOG_CAT, `Filtered ${tools.length} → ${filtered.length} tools based on intent`);
        return filtered;
    }
     ...
     */
    private scoreTools(tools: ToolDefinition[], message: string): Map<string, number> {
     ...
     */
    public prioritize(tools: ToolDefinition[], userMessage: string): ToolDefinition[] {
        if (!userMessage) return tools;
