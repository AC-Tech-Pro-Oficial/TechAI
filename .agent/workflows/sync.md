...
foreach ($p in $projects) {
    ...
    # PRESERVE README MANUAL SECTIONS
    $readmeManual = "Write your custom documentation here. It will be preserved."
    if (Test-Path "$path\README.md") {
        $existingReadme = Get-Content "$path\README.md" -Raw
        if ($existingReadme -match "(?s)<!-- MANUAL -->.*<!-- /MANUAL -->") {
            $readmeManual = $matches[0].Replace("<!-- MANUAL -->", "").Replace("<!-- /MANUAL -->", "").Trim()
        }
    }
